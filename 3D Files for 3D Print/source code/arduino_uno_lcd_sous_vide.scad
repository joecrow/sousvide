...
external_hole_rad=6.5/2;
external_temp_hole_rad=4/2;

part = "all"; // [ top, bottom, button, buttons, all ]
with_battery = 0; // [ 1:true, 0:false ] 
extra_room = 63; //20.6; // size of extra room, if any, set to 0 otherwise
...
module bottom() {
	...
	module sub() {
        ...
        if (extra_room > 0) translate([pcb[0]/2+tol+wall+extra_room/2, 0, wall]) {
            ...
            for (dx = [-5.0, 5.0]) translate([dx, pcb[1]/2 - 6.0/2, pcb2floor*.5]) rotate([0, -90, -90]) cylinder(r=external_hole_rad, h=extra_room, $fn=24); // holes in external wall
            for (dx = [-100]) translate([dx, pcb[1]/2 - 6.0/2, pcb2floor*.5]) rotate([0, -90, -90]) cylinder(r=external_temp_hole_rad, h=extra_room, $fn=24); // holes in external wall for temp probe
            difference () {
